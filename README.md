## ot2protocol
[![Build Status](https://travis-ci.org/pozzo-group-robots/OT2protocols2.svg?branch=master)](https://travis-ci.org/pozzo-group-robots/OT2Protocols2)
...
`ot2protocol/ot2protocol.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `ot2protocol.py` in directory called
`ot2protocol`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import ot2protocol as sb` in an
interactive Python session, the classes and functions defined inside of the
`ot2protocol.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the ot2protocol module, the main classes defined at the bottom of
...
Either way, you can create a `ot2protocol/data` folder in which you can
...
(typically, something like our `ot2protocol/tests/test_ot2protocol.py`). Within each
...
have one such test in `ot2protocol/tests/test_ot2protocol.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/ot2protocol`),
and type:
...
will see a message such as:
    ...
    ot2protocol/tests/test_ot2protocol.py .F...
    ...
    ot2protocol/tests/test_ot2protocol.py:49: AssertionError
...
To document `ot2protocol` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/ot2protocol/blob/master/doc/_static/RTD-advanced-conf.png)

 http://ot2protocol.readthedocs.org/en/latest/
...
A `ot2protocol/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/ot2protocol) for the software. This
...
`ot2protocol` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `ot2protocol`, we use the
...
[here](https://github.com/uwescience/ot2protocol/blob/master/scripts/Figure1.ipynb).
...
### Using `ot2protocol` as a template
...
`ot2protocol/ot2protocol.py` to be called `smallish/smallish.py`

	git mv ot2protocol smallish
	git mv smallish/ot2protocol.py smallish/smallish.py
	git mv smallish/tests/test_ot2protocol.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `ot2protocol` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `ot2protocol` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `ot2protocol` in them. Type
the following to see where all these files are:

	git grep ot2protocol

You can replace `ot2protocol` for `smallish` quickly with:

	git grep -l 'ot2protocol' | xargs sed -i 's/ot2protocol/smallish/g'
...
Edit all the mentions of `ot2protocol` in the `ot2protocol/__init__.py` file, and in
the `ot2protocol/version.py` file as well.
